import number  # import number.py file
...
class Block_type:
	def __init__(self, texture_manager, name="unknown", block_face_textures={"all": "cobblestone"}):
		...
		# set our block type's vertex positions, texture coordinates, and indices to the default values in our number.py file

		self.vertex_positions = number.vertex_positions
		self.tex_coords = number.tex_coords.copy()  # we need to create a copy of this, since we need to modify our texture coordinates in a different way for each block type (to have different textures per block)
